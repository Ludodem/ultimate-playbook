...
- [x] Terrain plein écran (mobile **et** desktop) : `.field-stage` occupe TOUT `.editor` (`position: absolute; inset: 0`), le terrain se dimensionne pour remplir cet espace en largeur **et** en hauteur (comme `object-fit: contain`, `Field.tsx`). Tout le reste de l'UI (topbar, menu ☰, toolbar d'ajout de joueur, timeline des frames, panneau de sélection, contrôles de lecture) flotte **par-dessus** en `position: fixed`, sous forme de pastilles translucides et floutées (`.overlay-bar`, effet "verre dépoli" via `backdrop-filter`) plutôt que de partager l'espace avec le terrain — voir journal de décisions `docs/ARCHITECTURE.md` §8. _Deux itérations avant d'arriver à cette version : (1) un premier essai en production a cassé le mobile ("interface complètement pétée") car validé avec un viewport de test artificiellement haut (844px au lieu de la hauteur réellement visible ~660-730px avec la barre d'adresse) — corrigé en donnant au terrain une vraie logique "tenir dans l'espace disponible" plutôt que de dériver sa hauteur de sa largeur seule ; (2) le correctif de l'itération 1 partageait encore l'espace vertical en flexbox (terrain + rangées de chrome empilées), ce qui a été jugé insuffisant par l'utilisateur ("je veux que le terrain PRENNE TOUT L'ECRAN... par dessus en overlay") — remplacé par la version actuelle où le terrain est une couche de fond plein écran et tout le chrome flotte dessus._
