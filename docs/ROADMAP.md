...
- [x] Terrain plein écran (mobile **et** desktop), reste de l'UI compacté sans scroll de page : nom/export/nouvelle action + toggle fantôme repliés derrière un menu ☰ (`editor-topbar`/`action-menu`), `.editor` en "app shell" plein écran (`height:100dvh`, colonne flex) à toutes les tailles d'écran, timeline des frames en bloc compact en bas (`max-height:26vh`, scroll interne, chips/boutons en scroll horizontal). _Correctif critique après premier essai en production (retour utilisateur "interface complètement pétée" sur mobile) : la validation initiale avait été faite sur un viewport Playwright artificiellement haut (844px, hauteur théorique de l'écran) au lieu de la hauteur réellement visible avec la barre d'adresse du navigateur (~660-730px selon l'appareil) — sur un vrai téléphone, le terrain (dérivé uniquement de la largeur, donc ~620px de haut) ne tenait pas du tout dans l'espace restant, forçant un scroll interne caché et non découvrable dans `.field-stage`. Corrigé à la racine dans `Field.tsx` : le terrain se dimensionne désormais pour tenir dans l'espace disponible en largeur **et** en hauteur (comme `object-fit: contain`), pas seulement en largeur — voir journal de décisions `docs/ARCHITECTURE.md` §8. Toolbar/hints restent en flux normal au-dessus du terrain (pas peints sur le canvas, pour ne jamais risquer de masquer un joueur), seul le menu ☰ est un vrai overlay._
