...
- **Marge sideline** (voir `docs/DATA_MODEL.md`, section "Marge sideline") : `computeVisibleXRangePercent(fieldConfig)` (dans `src/domain/geometry.ts`) renvoie la plage `{ min, max }` en % à rendre sur l'axe largeur (`{0,100}` par défaut, étendue si `sidelineMarginMeters > 0`). `toX` mappe cette plage — et non plus systématiquement `[0,100]` — sur la largeur du `Stage` ; le rectangle terrain, les en-buts et le contour de lignes sont dessinés sur `[toX(0), toX(100)]` uniquement, la zone hors-ligne (`colors.outOfBounds`) occupant le reste du canvas. Le rayon des entités/du disque reste calculé par rapport à la largeur _en jeu_ (`toX(100) - toX(0)`), pas à la largeur totale du `Stage`, pour que la taille des pastilles ne dépende pas du réglage de marge.
