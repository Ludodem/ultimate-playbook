...
- **Barre "Étapes" : toujours visible, en espace dédié (pas en recouvrement du terrain).** ("Frame"/"image-clé" reste le terme du modèle de données, voir §3 ; "Étape" est le libellé affiché à l'utilisateur — "Frames" ne voulait rien dire en français, retour utilisateur direct.) Elle affiche en permanence où on se trouve dans l'action (chemin depuis la racine, options à un embranchement) et permet de naviguer (taper une étape pour y aller) et d'enchaîner rapidement l'étape suivante — c'est l'action répétée en boucle en construisant un play, donc elle reste directement accessible. Les actions moins fréquentes (créer une branche, renommer une branche, réordonner, supprimer, ajouter une note) sont accessibles depuis le menu secondaire plutôt que d'alourdir cette barre.
...
### 4.8ter Panneau Étapes latéral (grand écran)

Sur PC/tablette large, une fois le terrain ajusté à l'espace disponible (§4.8), il reste souvent un espace inutilisé sur les côtés (le terrain est contraint par la hauteur, pas par la largeur). Cet espace est alors réservé à une **colonne latérale "Étapes"**, qui remplace la barre du bas plutôt que de s'y ajouter : elle montre **l'arbre complet** de l'action (toutes les branches, pas seulement le chemin courant), avec indentation et couleur par branche, cliquable directement pour naviguer vers n'importe quelle étape en un seul geste — un avantage sur la barre du bas, qui ne montre que le chemin courant. Bascule entre les deux vues basée sur une **mesure réelle de l'espace disponible** (pas un point de rupture d'écran fixe) : si le terrain laisse au moins ~280px de côté une fois ajusté à l'orientation courante, le panneau latéral s'affiche ; sinon, la barre du bas (§4.8) reste utilisée. Les actions rapides (étape suivante, nouvelle branche) restent disponibles dans les deux vues.

### 4.8quater Menu déployé en permanence (grand écran)

Sur le même critère d'espace mesuré que §4.8ter (et avec le même panneau latéral) : le contenu du menu secondaire ⋯ (nom de l'action, export, ajout de joueur, undo/redo, orientation, étape fantôme, actions d'étape) s'affiche **en permanence** dans la colonne latérale, au-dessus du panneau Étapes, plutôt que derrière un bouton à ouvrir — puisque la place ne manque pas, autant rendre les options directement cliquables sans étape intermédiaire. Le bouton ⋯ lui-même disparaît dans ce mode (il n'a plus de rôle). Sur écran étroit (smartphone, ou grand écran en orientation paysage si le terrain y occupe alors presque toute la largeur), le comportement d'origine (§4.8) reste inchangé : bouton ⋯ replié, panneau flottant à l'ouverture.
